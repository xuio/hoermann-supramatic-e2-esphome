...
Because Home Assistant may hide the position slider for a `device_class: garage` cover, the primary YAML also exposes `Garage Door Target Position` as a number entity. Use that slider for percentage tests. On this E2, HCP movement broadcasts can decode as `0x0000` while the door is physically moving, so percentage stopping requires the configured raw stop path (`use_unverified_stop_command: true`). `Garage Door Open Duration`, `Garage Door Close Duration`, and the start/report delay number entities show the active calibration values in seconds. Automatic duration learning is disabled in the E2 YAML so the video-derived timing values remain authoritative.
