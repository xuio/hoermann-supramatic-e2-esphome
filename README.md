...
This allows receive, scan response, state decoding, light, venting, and impulse testing while blocking remote close. After state decoding and physical safety behavior are verified at the door, set `allow_remote_close: true` to enable Home Assistant close commands.
...
When configuring HomeKit Bridge, include only the garage cover and optionally the garage light. Exclude the impulse button, venting switch, diagnostics, raw state helpers, and error/prewarn sensors from HomeKit.
