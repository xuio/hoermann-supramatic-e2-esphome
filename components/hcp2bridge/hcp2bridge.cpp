...
bool HCP2Bridge::apply_drive_status_locked_(const hcp2_drive_status_t *status, bool valid_broadcast_seen,
                                            uint32_t now_ms) {
  bool changed = false;
  ...
  if (!this->valid_broadcast_ && valid_broadcast_seen) {
...
void HCP2Bridge::update_state_from_engine_() {
  ...
  portENTER_CRITICAL(&this->state_mux_);
  changed = this->apply_drive_status_locked_(status, this->engine_.broadcasts_received > 0, now_ms);
...
void HCP2Bridge::update_state_from_mailbox_() {
  ...
  portENTER_CRITICAL(&this->state_mux_);
  changed = this->apply_drive_status_locked_(have_state ? &status : nullptr, have_state && snapshot.updated_us != 0u,
                                             now_ms);
